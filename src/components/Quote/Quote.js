	...
	const fetchQuoteByCategory = async (e) => {
		...
		try {
			const response = await axios.get(
				`https://api.quotable.io/random?tags=${searchTag}`
			);
			setQuote(response?.data);
			setLikes(0);
		} catch (error) {
		...
		<div className="quote-wrapper">
			<form className="quote-searching-feature">
				<input
					...
					placeholder="Enter category (e.g. inspiration)"
				/>
				<button
					className="homepage-search-button"
					type="submit"
					onClick={fetchQuoteByCategory}
				>
					Search
				</button>
			</form>
			...
			<div className="quote-features">
				<div className="quote-like-section">
					...
					<Badge color="success" badgeContent={likes}></Badge>
				</div>
				<div className="quote-extra-info-wrapper">
					<span className="quote-author">- {quote?.author}</span>
					<span className="quote-topic">
						<span className="quote-topic-name">Related Topic:</span>{" "}
						{quote?.tags?.[0]}
					</span>
				</div>
			</div>
